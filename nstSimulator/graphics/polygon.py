...
class Polygon3d():
    ...
    def makeNode(self, trangulation_plane="xy"):
        vt = self.vertices
        vc = self.color
        t = Triangulator()
        format = GeomVertexFormat.getV3c4()
        ...
        color = GeomVertexWriter(vdata, 'color')
        for x,y,z in vt:
            if trangulation_plane == "xy":
                t.addPolygonVertex(t.addVertex(x,y))
            elif trangulation_plane == "xz":
                t.addPolygonVertex(t.addVertex(x,z))
            elif trangulation_plane == "yz":
                t.addPolygonVertex(t.addVertex(y,z))
