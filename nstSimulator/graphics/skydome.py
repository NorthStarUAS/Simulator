...
class GroundBowl():
    def __init__(self):
        ...
        self.ground = render.attachNewNode("ground")

        # varc = gen_arc(r1, start_deg=0, end_deg=90, divs=9)
        vangles = [0,  -2,  -4,   -8, -20,  -40, -60, -90]
        vblend =  [1, 0.8, 0.5, 0.15, 0.1, 0.05, 0.0, 0.0]
        varc = gen_arc_list(r1, angle_list=vangles)
        print("varc:", varc)
        arc_list = []
        for x1, y1 in varc[:-1]:
            ...
            # arc = ArcLine2d(color4=(1, 1, 1, 1), radius=y1*0.95, width=1, steps=36)
            # arc.group_node.setPos(0, 0, x1)
            # arc.group_node.reparentTo(self.ground)
        ...
        for i in range(len(arc_list)-1):
            ...
            vdata = GeomVertexData("ground", ground_format, Geom.UHStatic)
            vdata.setNumRows(len(arc1)*2)
            ...
            blend1 = vblend[i]
            blend2 = vblend[i+1]
            color1 = fog * blend1 + brown * (1-blend1)
            color2 = fog * blend2 + brown * (1-blend2)
            color_writer = GeomVertexWriter(vdata, 'color')
            for j in range(len(arc1)):
                color_writer.addData4(color1)
                color_writer.addData4(color2)
            ...
            surface = self.ground.attachNewNode(node)
            surface.setTwoSided(True)
            # surface.setMaterial(ribbonMat)
            # surface.setTexture(tex)
            # surface.setTransparency(TransparencyAttrib.MAlpha)
        ...
        # self.ground.setMaterial(compassMat)
        # self.ground.setTexture(tex)
        # self.ground.setTransparency(TransparencyAttrib.MAlpha)
        self.ground.setDepthWrite(False)
        self.ground.setDepthTest(False)
        # node.setDepthWrite(False)
        # node.setDepthTest(False)
        self.ground.setBin("background", 0)

    def update(self, nedpos):
        self.ground.setPos(nedpos[1], nedpos[0], -nedpos[2])
