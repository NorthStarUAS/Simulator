...
class ArcLine2d():
    def __init__(self, color4=(0.9, 0.1, 0.1, 0.7), radius=1, width=1, start_deg=0, end_deg=360, steps=10, font_scale=(0.1, 0.3), tic_list=[], mark_list=[], label_radius=1, label_list=[]):
        ...
        # generate individual tics
        for [tic_angle, tic_scale] in mark_list:
            tic_outer = gen_arc_list(radius, [tic_angle])
            tic_inner = gen_arc_list(radius*tic_scale, [tic_angle])
            (x1, y1) = tic_outer[0]
            (x2, y2) = tic_inner[0]
            ls.moveTo(x1, y1, 0)
            ls.drawTo(x2, y2, 0)
