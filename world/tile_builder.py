                        ...
                        for i in range(len(fit_pts)):
                            if zs[i] > -10000:
                                ...
                                if is_skirt[i]:
                                    ...
                                    fit_vals[i] -= xsize_m / 10
                time.sleep(0)

        # fixme: default val zero vs. /down/ elevation quick sanity check
        for i in range(len(fit_vals)):
            if fit_vals[i] is None:
                print("Problem interpolating elevation for:", fit_pts[i])
                fit_vals[i] = 0.0 # ocean?

        fv = np.array(fit_vals)
        sk = np.array(is_skirt)
        avg_elev_m = np.mean(fv[sk==False])
        # print(is_skirt, fv[sk==False])
        print("Tile average elev (m):", avg_elev_m)

        return fit_pts, fit_vals, [center_lat, center_lon, avg_elev_m], texcoords, is_skirt
